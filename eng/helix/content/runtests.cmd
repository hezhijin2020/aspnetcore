...
if %$quarantined%==true (
    ...
    %DOTNET_ROOT%\dotnet vstest %$target% --settings helixtests.runsettings --logger:xunit --TestCaseFilter:%QUARANTINE_FILTER%
    if errorlevel 1 (
    ...
    %DOTNET_ROOT%\dotnet vstest %$target% --settings helixtests.runsettings --logger:xunit --TestCaseFilter:%NONQUARANTINE_FILTER%
    if errorlevel 1 (
